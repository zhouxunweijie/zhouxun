/**
 * ajax:封装一个公共方法，用来实现AJAX的数据请求
 * @parameter:
 *  options:[object]
 * @return:
 *
 * by ream on 2016-11-17 12:43:00
 */
function ajax(options) {
    ...
    var _default = {
        ...
        dataType: 'text',//->服务器返回的都是字符串,我们这个参数是控制需不需要把返回的内容解析成其它的方式 txt、json、xml...
...
/*
* 当我们封装的方法需要传递的参数过多的时候，我们把其定义成一个个形参的方式不好：必须都要传递，有一个不传递后面的值就乱套了，传地的顺序也必须保持一致，如果后期再升级的话，还需要在后面加参数
* 解决方案：
*   我们只给这个方法传递一个参数值options，这个参数是一个对象数据类型的，我们把需要配置的信息都已键值对的方式储存到这个对象中
*
* */